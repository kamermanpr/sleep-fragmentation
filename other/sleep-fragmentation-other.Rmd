...
# Friedman test - Before vs during ischaemia
## Baseline
vf_base <- vf_plot %>%
    ungroup() %>%
    ...
    mutate(period = factor(period),
           time = factor(time)) 
vf_base_F <- friedman.test(score ~ time | id, data = vf_base); vf_base_F
...
vf_frag1 <- vf_plot %>%
    ...
    mutate(period = factor(period),
           time = factor(time)) 
vf_frag1_F <- friedman.test(score ~ time | id, data = vf_frag1); vf_frag1_F
...
vf_frag2 <- vf_plot %>%
    ...
    mutate(period = factor(period),
           time = factor(time)) 
vf_frag2_F <- friedman.test(score ~ time | id, data = vf_frag2); vf_frag2_F
## Holm correction
